...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; }
        html, body { margin:0; padding:0; font-family:'Poppins',sans-serif; background:#f8f5f7; color:#333; height:100%; }
        .wrapper { display:flex; min-height:100vh; }
        #sidebar { width:var(--sidebar-w); min-width:var(--sidebar-w); flex-shrink:0; background:linear-gradient(180deg,var(--salon-dark) 0%,#1a1a2e 100%); display:flex; flex-direction:column; }
        ...
        .nav-section-label { font-size:.62rem; font-weight:600; letter-spacing:1.5px; text-transform:uppercase; color:rgba(255,255,255,.3); padding:.75rem 1.25rem .25rem; }
        .sidebar-nav a { display:flex; align-items:center; gap:.75rem; color:rgba(255,255,255,.6); padding:.65rem 1.25rem; text-decoration:none; font-size:.875rem; transition:all .2s; border-left:3px solid transparent; }
        .sidebar-nav a i { font-size:1.05rem; }
        .sidebar-nav a:hover, .sidebar-nav a.active { color:#fff; background:rgba(240,98,146,.18); border-left-color:var(--salon-pink); }
        ...
        .stat-card { border-radius:12px; padding:1.25rem 1.5rem; color:#fff; position:relative; overflow:hidden; min-height:110px; margin-bottom:0; }
        .stat-card .icon { font-size:2.8rem; opacity:.2; position:absolute; right:1rem; bottom:.5rem; }
        .stat-card .stat-value { font-size:1.9rem; font-weight:700; line-height:1.1; }
        .stat-card .stat-label { font-size:.78rem; opacity:.85; margin-bottom:.25rem; }
        ...
        .table th { font-size:.78rem; font-weight:600; text-transform:uppercase; letter-spacing:.5px; color:#999; }
